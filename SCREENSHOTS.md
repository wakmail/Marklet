# More screenshots

Full window captures of Marklet, with different themes and writing styles. Select an image to see it at full size.

## Marklet Warm

Warm paper colors with purple and teal accents, live formatting, highlights, and checklists.

[![Marklet Warm showing formatted writing and a checklist](media/screenshots/01-warm-writing.png)](media/screenshots/01-warm-writing.png)

## Catppuccin Mocha math

Display equations and inline LaTeX in Catppuccin's dark Mocha palette.

[![Catppuccin Mocha showing inline math and two rendered equations](media/screenshots/04-catppuccin-math.png)](media/screenshots/04-catppuccin-math.png)

## Paper reading

A quiet reading view with the Paper palette and Georgia typography.

[![The Paper theme showing a prose document in Reading mode](media/screenshots/06-paper-reading.png)](media/screenshots/06-paper-reading.png)

[Back to Marklet](README.md)
